...
module.exports = function (deployer, network, accounts) {
  deployer.then(async ()=>{
    ...
    if (network == "ropsten" || network == "bsctestnet") {//console.log("deploy ropsten LnFeeSystemTest");
      ...
      await CallWithEstimateGas(kLnFeeSystem.Init, kLnExchangeSystem.address, kLnRewardLocker.address);
      ...
      let old = await LnFeeSystemTest.at(kLnFeeSystem.address);
      ...
      await CallWithEstimateGas(kLnFeeSystem.Init, exchangeAddress, kLnRewardLocker.address);
    }
 ...
 // deploy a new LnChainLinkPrices and update to other referenced contract.
    ...
    await CallWithEstimateGas(kLnCollateralSystem.UpdateTokenInfo, 
      ...
      console.log("oracle", oracle, admin);
      await CallWithEstimateGas(kLnChainLinkPrices.updateAll, 
        [lBTC32, lETH32, lHB1032, lUSD32],
        [toUnit(17140.02), toUnit(455.4), toUnit(1.4), toUnit(6.4)],
        Math.floor(Date.now()/1000).toString()
      );
